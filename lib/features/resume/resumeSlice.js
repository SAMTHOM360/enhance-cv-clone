...
export const resumeSlice = createSlice({
  ...
  reducers: {
    ...
    toggleHeaderFieldVisibility: (state, action) => {
      saveToHistory(state)
      const { field, value } = action.payload
      state.header.visibility[field] = value
    },

    toggleUppercaseName: (state, action) => {
      saveToHistory(state)
      state.header.uppercaseName = action.payload.value
    },

    togglePhotoStyle: (state, action) => {
      saveToHistory(state)
    ...
    upsertActiveSection: (state, action) => {
    ...
    removeSectionEntry: (state, action) => {
      saveToHistory(state)
    ...
    updateSectionColumn: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section) {
    ...
    // Education actions
    addEntryEducation: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
    ...
    removeEntryEducation: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section && section.content.educations) {
        section.content.educations = section.content.educations.filter((e) => e.id !== action.payload.entryId)
    ...
    updateEntryEducation: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
    ...
    toggleEntryVisibility_Education: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section && section.content.educations) {
        const entry = section.content.educations.find((e) => e.id === action.payload.entryId)
    ...
    removeEntryProject: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section && section.content.projects) {
    ...
    updateEntryProject: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section && section.content.projects) {
    ...
    toggleEntryVisibility_Project: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section && section.content.projects) {
    ...
    // Skills actions
    addEntrySkillGroup: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
    ...
    updateEntrySkillGroup: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section && section.content.skills) {
    ...
    removeEntrySkillGroup: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section && section.content.skills) {
    ...
    setActiveSkillData: (state, action) => {
      if (action.payload) {
    ...
    toggleEntryVisibility_SkillsContent: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section && section.content.skills) {
    ...
    updateEntryLanguage: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section && section.content.languages) {
        const language = section.content.languages.find((l) => l.id === action.payload.langId)
        if (language) {
          language[action.payload.field] = action.payload.value
        }
      }
    },

    removeEntryLanguage: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section && section.content.languages) {
    ...
    updateAchievement: (state, action) => {
      ...
      if (section?.content.achievements) {
        ...
        if (achievement) {
          achievement[action.payload.field] = action.payload.value
        }
      }
    },
    ...
    toggleEntryVisibility_Achievement: (state, action) => {
      saveToHistory(state)
      const section = state.sections.find((s) => s.id === action.payload.sectionId)
      if (section?.content.achievements) {
        const achievement = section.content.achievements.find((a) => a.id === action.payload.entryId)
        if (achievement?.visibility) {
          achievement.visibility[action.payload.field] = action.payload.value
        }
      }
    }
  },
