...
import { proficiencyLabels } from "@/lib/types"

export default function LanguageSection({ section, isActive, darkMode = false, handleEntryToggle, handleContextMenu }) {
    const dispatch = useDispatch()
    const activeSection = useSelector((state) => state.resume.activeSection)

    const handleEntryUpdate = (langId, field, value) => {
        dispatch(
            updateEntryLanguage({
    ...
    return (
        <div className="Language-Section space-y-1">
            {section.content.languages?.map((item) => (
                <div
                    key={item.id}
                    className={cn(
