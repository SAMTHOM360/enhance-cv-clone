...
export default function AchievementsSection({ section, isActive, darkMode = false, handleEntryToggle, handleContextMenu }) {
    const dispatch = useDispatch()
    const activeSection = useSelector((state) => state.resume.activeSection)

    const handleEntryUpdate = (entryId, field, value) => {
        dispatch(
            updateAchievement({
    ...
    return (
        <div className="Achievements-Section space-y-4">
            {section.content.achievements?.map((achievement) => (
