...
export default function ResumeSection({ section, isActive, onDragStart, darkMode = false }) {
    const dispatch = useDispatch()
    const activeSection = useSelector((state) => state.resume.activeSection)
    const { activeSkillData } = useSelector((state) => state.resume)
    ...
    const handleEntryToggle = (e, entryId) => {
        e.stopPropagation()
        dispatch(upsertActiveSection({
            activeSection: {
                id: section.id,
    ...
    const handleContextMenu = (e, entryId, groupId) => {
        e.preventDefault()
        if (entryId) {
            dispatch(upsertActiveSection({
                activeSection: {
    ...
    const handleUpdateSectionChange = (newTitle) => {
        dispatch(
            updateSectionTitle({
    ...
    const handleAddEntry = () => {
        ...
        switch (section.type) {
            case SectionTypeEnum.EDUCATION:
                dispatch(addEntryEducation({ sectionId: section.id, education: entry }))
                break
            case SectionTypeEnum.PROJECTS:
                dispatch(addEntryProject({ sectionId: section.id, project: entry }))
                break
            case SectionTypeEnum.LANGUAGES:
                dispatch(addEntryLanguage({ sectionId: section.id, language: entry }))
                break
            ...
            case SectionTypeEnum.ACHIEVEMENTS:
                dispatch(addAchievement({ sectionId: section.id, achievement: entry }))
                break
        }
    }
    ...
    const handleToggleVisibility = (field, value) => {
        if (activeSection?.entryId && section.type in visibilityDispatchMap) {
            const actionCreator = visibilityDispatchMap[section.type]
    ...
    useEffect(() => {
        if (!isActive) return
        const handleClickOutside__Section = (event) => {
